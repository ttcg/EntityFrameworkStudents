...
namespace Students.IntegrationTests.Students.Services
{
    public class WhenWithdrawingCourse : IClassFixture<IntegrationTestFactory>
    {
        ...
        [Fact]
        public async Task ShouldWithdrawInProgressCourse()
        {
            var result = await _studentService.WithdrawCourse(_student1.StudentId, Course.Physics);
        ...
        [Fact]
        public async Task ShouldNotWithdraw_AlreadyCompletedCourse()
        {
            var result = await _studentService.WithdrawCourse(_student1.StudentId, Course.Maths);
            Assert.NotNull(result);
        ...
        [Fact]
        public async Task ShouldNotWithdraw_WithdrawnCourse()
        {
            var result = await _studentService.WithdrawCourse(_student1.StudentId, Course.Chemistry);
            Assert.NotNull(result);
            Assert.True(result.IsFailure);
            Assert.Equal(StudentErrors.Constants.EnrolmentNotFound, result.Error.Code);
        }
        ...
        [Fact]
        public async Task ShouldNotWithdrawWithoutEnrolment()
        {
            var result = await _studentService.WithdrawCourse(_student1.StudentId, Course.Bio);
            Assert.NotNull(result);
            Assert.True(result.IsFailure);
            Assert.Equal(StudentErrors.Constants.EnrolmentNotFound, result.Error.Code);
        }
        ...
        {
            var result = await _studentService.WithdrawCourse(999, Course.Maths);
            Assert.NotNull(result);
            Assert.True(result.IsFailure);
